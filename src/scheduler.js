...
export class Scheduler extends EventEmitter {
  ...
  ready(account) {
    return account.apiKeyEnv
      ? Boolean(process.env[account.apiKeyEnv])
      : existsSync(
          path.join(this.config.dataDir, 'accounts', account.id, 'auth.json'),
        );
  }
  accountStatus(account) {
    ...
    if ([...this.active.values()].some((a) => a.accountId === account.id))
      return 'busy';
    return 'ready';
  }
  changed() {
  ...
  tick() {
    ...
    for (const job of this.store.state.jobs) {
      ...
      if (
        [...this.active.values()].some((a) =>
          overlaps(a.workspace, job.workspace),
        )
      )
        continue;
      const candidates = this.store.state.accounts.filter(
        (a) =>
          this.accountStatus(a) === 'ready' &&
          (!job.accountId ||
            a.id === job.accountId ||
            (job.autoFailover && job.attempts.length > 0)),
      );
  ...
  start(job, account) {
    ...
    Promise.resolve()
      ...
      .then((result) => {
        ...
        attempt.status = controller.signal.aborted
          ? 'cancelled'
          : result.ok
            ? 'completed'
            : result.quota
        ...
        if (result.quota) {
          account.cooldownUntil = Date.now() + this.config.cooldownMs;
        ...
        if (controller.signal.aborted) job.status = 'cancelled';
          ...
          result.quota &&
          job.autoFailover &&
          job.attempts.length < this.config.maxAttempts
        ) {
          if (job.sandbox === 'workspace-write' && !job.allowWriteRetry) {
            job.status = 'needs_review';
            job.error =
              'Quota reached after a write-enabled attempt. Review the workspace, then retry or enable write retries on a new job.';
          } else {
            job.status = 'waiting';
          ...
          job.status = 'failed';
          job.error = result.error || 'Codex failed.';
        }
  ...
  cancel(job) {
    if (!['queued', 'waiting', 'running'].includes(job.status))
      throw Object.assign(new Error('This job is no longer active.'), {
        status: 409,
      });
